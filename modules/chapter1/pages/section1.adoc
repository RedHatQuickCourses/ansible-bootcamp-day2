...
WARNING: In most of the labs, we provided many of the values that were required to simplify the experience. However, in this lab, you will be required to specify the majority of the values based on your environment. This approach will enable you to better understand how the values apply to Configuration as Code.
...
Create a new `execution-environment.yml` file within the `config_as_code` directory with the following content and change the 'openshift_cluster_ingress_domain' and '<AAP-hostname-from-rhdp-login-page>' with the actual values before saving the file:
...
images:
  base_image:
    name: <AAP-hostname-from-rhdp-login-page>/ansible-automation-platform-25/ee-minimal-rhel9:latest
...
additional_build_steps:
  prepend_galaxy:
    ...
    - ENV ANSIBLE_GALAXY_SERVER_CERTIFIED_URL='<AAP-hostname-from-rhdp-login-page>/pulp_ansible/galaxy/rh-certified/'
    - ENV ANSIBLE_GALAXY_SERVER_CERTIFIED_TOKEN=$TOKEN
    - ENV ANSIBLE_GALAXY_SERVER_VALIDATED_URL='<AAP-hostname-from-rhdp-login-page>/pulp_ansible/galaxy/validated/'
    - ENV ANSIBLE_GALAXY_SERVER_VALIDATED_TOKEN=$TOKEN
    - ENV ANSIBLE_GALAXY_SERVER_COMMUNITY_URL='<AAP-hostname-from-rhdp-login-page>/pulp_ansible/galaxy/community/'
    - ENV ANSIBLE_GALAXY_SERVER_COMMUNITY_TOKEN=$TOKEN
    - ENV ANSIBLE_GALAXY_SERVER_PUBLISHED_URL='<AAP-hostname-from-rhdp-login-page>/pulp_ansible/galaxy/published/'
...
podman login <AAP-hostname-from-rhdp-login-page>
...
podman tag localhost/config_as_code_ee:1.0 <AAP-hostname-from-rhdp-login-page>/config_as_code_ee:1.0
podman push <AAP-hostname-from-rhdp-login-page>/config_as_code_ee:1.0
----
...
IMPORTANT: Update the value of `<AAP_HOST>` to match the location of your AAP instance (`<AAP-hostname-from-rhdp-login-page>`).
...
aap_service_account_username: "< student_account >"
aap_service_account_password: "< aap_password >"
...
[source,bash,role=execute,subs="verbatim,attributes"]
----
ansible-navigator run --pp=missing --eei=<AAP-hostname-from-rhdp-login-page>/config_as_code_ee:1.0 -m stdout playbooks/aap_config.yml -e env=dev
----
...
image::hub_task.png[Hub task,125%,125%,link=self, window=blank]
...
image::config_ee_v2.png[title="Config Execution Environment",125%,125%, link=self, window=blank]
...
image::credential_gitea_v2.png[title="Credential",125%,125%, link=self, window=blank]
...
image::orgs.png[title="Config as Code Organization",125%,125%, link=self, window=blank]
...
image::project_v2.png[title="Project",125%,125%, link=self, window=_blank]
...
image::inventories_v2.png[title="Inventory",125%,125%, link=self, window=_blank]
...
image::inventory_sources_v2.png[title="Inventory Sources",125%,125%, link=self, window=_blank]
...
image::aap_config_template_v3.png[title="AAP Config Template",125%,125%, link=self, window=_blank]
